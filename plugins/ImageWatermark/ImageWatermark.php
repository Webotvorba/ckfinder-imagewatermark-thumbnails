...
class ImageWatermark implements PluginInterface, EventSubscriberInterface
{
    ...
    {
        return [
            'imagePath' => __DIR__ . '/wm.png', // Also use a custom image.
            'position' => [
                'right'  => 0,
                'bottom' => 0
            ]
    ...
    public function addWatermark(FileUploadEvent $event)
    {
        ...
        if (Image::isSupportedExtension($uploadedFile->getExtension()) && $uploadedFile->isValidImage() && $_COOKIE['thumbnail'] == 1)
        {
            ...
            if ($height > $this->app['config']->get('ImageWatermark.minHeight') && $_COOKIE['watermark'] == 1)
			{
				...
				if (Image::isSupportedExtension(pathinfo($watermarkImagePath, PATHINFO_EXTENSION)))
				{
					...
					$uploadedFile->save($uploadedImage->getData(null, $this->app['config']->get('images.quality')));
			...
			// Resize image
            $path        = $this->app['config']->get('Thumbnail.baseDir')."/../uploads/".$this->app->getWorkingFolder()->getPath()."thumbs/";
            $thumbWidth  = $this->app['config']->get('Thumbnail.thumbWidth');
            $thumbHeight = $this->app['config']->get('Thumbnail.thumbHeight');

            // Create dir if it doesnt exist
            if (!file_exists($path))
                mkdir($path);

            $thumbImage  = Image::create($uploadedFile->getContents());
            $thumbImage  = $thumbImage->resize($thumbWidth, $thumbHeight);
            file_put_contents($path.$uploadedFile->getFilename(), $thumbImage->getData(null, $this->app['config']->get('images.quality')));
